...
from apps.telegram.app.clients.tracefold_api import TraceFoldApiClient, TraceFoldApiError
...
def test_tracefold_api_client_maps_error_envelope_to_adapter_error():
    transport = httpx.MockTransport(
        lambda request: httpx.Response(
            ...
            json={
                ...
                "data": None,
                "error": {"code": "PENDING_ITEM_NOT_FOUND"},
                "meta": None,
            },
        )
    )
    ...
    with pytest.raises(TraceFoldApiError) as exc_info:
        api_client.request("GET", "/pending/999")

    assert exc_info.value.status_code == 404
    assert exc_info.value.error_code == "PENDING_ITEM_NOT_FOUND"
