...
但请注意：当前仓库的**正式启动入口**还没有收成一个长期运行的 polling daemon，这一点会在下面的 TraceFold 专用部分明确说明。
...
- `docs/STEP7_CH2_ACCEPTANCE_SMOKE.md`
...
也就是说，**当前正式入口是 startup probe，不是长期运行的 polling daemon**。
...
但当前正式入口还没有把它们收成一个常驻 polling runtime。

所以这份教程会分清两件事：

1. 当前仓库已经支持并测试的 Telegram 命令面
2. 当前正式入口实际上只做到“启动探测/接线确认”，还没有做成稳定常驻 bot runtime
...
那么按**当前仓库现实**，这一步还没有被正式收成日常推荐运行入口。

当前已经实现的是：

- adapter 命令语义
- API client
- `getUpdates` / `process_updates_once()` 方向
- 测试覆盖

当前还没有正式收成的是：

- 长期运行的、文档化的 polling bot runtime

这不是 Telegram 职责边界的问题，而是当前仓库实现层级的现实限制。
...
按当前 tests 和 handlers 的现实，这些命令都属于 lightweight adapter 范围，不代表 Telegram 已经成为完整运行工作台。
...
只做依赖探测，不会长期跑 `getUpdates` 循环。

所以如果你问“为什么我给 bot 发命令，它没持续回复”，要先确认你理解的是仓库现实，而不是理想中的 bot daemon。

### 8.5 Telegram 命令能发出，但 TraceFold 后端不可达

当前 handler/formatter 的短错误反馈大致会是：

- `Service status unavailable. Try again later.`
...
- lightweight adapter
- quick capture / quick check surface
- 统一 HTTP API 的轻入口
...
- 当前正式入口是 probe-only
- 还没有把常驻 polling runtime 收成正式推荐运行入口
...
8. 记住当前现实边界：
   - 命令面已经实现并测试
   - 正式入口当前只做 probe
   - 还没有正式收成长期开着的 polling bot runtime
